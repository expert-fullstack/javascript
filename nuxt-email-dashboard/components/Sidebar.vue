<template>
  <NuxtScrollbar
    tag="aside"
    class="fixed h-screen hidden md:block md:w-72 border-r border-gray-200 bg-zinc-100 shrink-0"
  >
    <nav class="flex flex-col gap-2 px-4 mt-10">
      <NuxtLink
        class="flex group items-center justify-between px-3 py-2 rounded-lg text-zinc-600 hover:text-primary-50 hover:bg-primary transition"
        :to="ml.link"
    ...
    <div class="px-4 my-10">
      ...
      >
        <Icon
          ...
          class="transition duration-300 text-zinc-600"
          name="solar:alt-arrow-down-bold-duotone"
          size="18"
        />
        <span class="text-sm text-zinc-600">Labels</span>
      </button>
      <Transition name="labels">
        <nav v-if="showLabels" class="flex flex-col gap-1 mt-5">
          <NuxtLink
            ...
            v-for="l in labels"
            :key="l.title"
            class="flex text-zinc-600 items-center justify-between rounded-lg p-2 hover:bg-zinc-200 transition"
          >
